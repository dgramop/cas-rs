...
fn fmt_non_normal_decimal<F: std::fmt::Write>(f: &mut F, n: f64) -> std::fmt::Result {
    if n.is_normal() {
...
fn fmt_decimal<F: std::fmt::Write>(f: &mut F, n: f64, options: FormatOptions) -> std::fmt::Result {
    //TODO(dgramop) respect function's intent
    n.fmt(f)
}

/// Formats a float in scientific notation.
pub fn fmt_scientific(f: &mut Formatter<'_>, n: f64, options: FormatOptions) -> std::fmt::Result {
    n.fmt(f)
}

/// Formats a float as a rational fraction.
fn fmt_fraction(f: &mut Formatter<'_>, n: f64, options: FormatOptions) -> std::fmt::Result {
    if !n.is_normal() {
...
fn fmt_word(f: &mut Formatter<'_>, n: f64, options: FormatOptions) -> std::fmt::Result {
    if !n.is_normal() {
        return fmt_non_normal_decimal(f, n);
    }
...
pub fn fmt(f: &mut Formatter<'_>, n: f64, options: FormatOptions) -> std::fmt::Result {
